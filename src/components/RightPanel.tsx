...
// Import images (these are objects with a `src` property in Next.js)
import mitCertificateImg from '../assets/images/MIT Certificate.png';
import davenportUniversityImg from '../assets/images/Davenport University.png';
...
function FloatingStar({ position }: FloatingStarProps) {
  ...
  useFrame(({ clock }) => {
    if (ref.current) {
      ref.current.position.y = position[1] + Math.sin(clock.getElapsedTime() * 1.2) * 0.1;
  ...
  return (
    <group ref={ref} position={position}>
      <mesh castShadow receiveShadow>
        <extrudeGeometry
          args={[starShape, { depth: 1, bevelEnabled: true, bevelThickness: 0.18, bevelSegments: 4 }]}
        />
      ...
      >
        <div style={{
          ...
          fontSize: '2em',
          color: '#fff',
          textShadow: '0 0 16px #ff6a13, 0 0 14px #ffbe29, 0 0 12px #FF7300, 0 0 6px #fd2d00',
          lineHeight: 1,
...
const AnimatedStepGroup = ({
  visible, children, index, animateDuration = 0.5,
  ...
  useFrame((_, delta) => {
    ...
    groupRef.current.position.x = current + (targetX - current) * Math.min(1, delta * 16 / animateDuration);
  });
  useEffect(() => {
    if (!groupRef.current) return;
...
const StairStep = ({
  color, position, index, topTilt = 0, topLabel, frontLabel, showTop, showFront,
}: StairStepProps) => {
  ...
  // Increase extrusion depth for the top step and shift down half the added thickness for visual alignment
  const isTopStep = index === stepData.length - 1;
...
const imageHeight = STEP_HEIGHT * 1.5; 
  ...
  // Select image for first two steps
  const imageSrc =
    index === 0
      ? mitCertificateImg.src
      : index === 1
      ? davenportUniversityImg.src
      : null;

  return (
    <group position={position}>
      {/* Top face (with tilt and adjusted thickness for the top step) */}
      <mesh
      ...
      <mesh position={[0, STEP_HEIGHT / 2, STEP_DEPTH / 2 + 0.13]} castShadow receiveShadow>
        ...
        {showFront && imageSrc && (
          <Html
            ...
            position={[0, 0, 0.17]} // place just above the front face
            transform
            zIndexRange={[11, 0]}
          >
            <img
              ...
              style={{
                ...
                border: '4px solid brown',   // <-- Add this line for brown border
                background: '#fff',
      ...
      {/* Left side - start at step top, extend straight down */}
      <mesh
...
const RightPanel = () => {
  ...
  return (
    ...
    >
      ...
      >
        <FullStaircaseScene steps={steps} revealedCount={revealedSteps} showStar={showStar} />
      </div>
